...
@Component({
  selector: 'app-skyfair',
  templateUrl: './skyfair.component.html',
  styleUrls: ['./skyfair.component.scss']
})
export class SkyfairComponent implements OnInit {
  ...
  providerList = [
    { "providerName": "Supernowa", "providerCode": "SN", "isBig": false },
  ];
  snGameAssetsAll = [
  ...
  awcCasinoList_9ex = [
    ...
    { sr: 9, prod_code: '59', prod_type: '2', name: "Rummy", prod_name: "JILI", game_code: 'JILI-TABLE-004', prod_type_name: 'SLOT' },
    { sr: 9, prod_code: '117', prod_type: '2', name: "Horse Racing", prod_name: "KINGMAKER", game_code: 'KM-TABLE-048', prod_type_name: 'SLOT' , isBig : true },
    ...
    { sr: 9, prod_code: '117', prod_type: '2', name: "Teenpatti", prod_name: "KINGMAKER", game_code: 'KM-TABLE-046', prod_type_name: 'SLOT' },
  ...
  listCasinoProduct() {
    ...
    this.apiService.listCasinoProduct().subscribe((resp: any) => {
      if (resp.result) {
        let awcCasinoList_9ex = [];
        let awcCasinoList_skyfair = [];



        this.awcCasinoList_9ex.forEach(element2 => {
          resp.result.forEach((element, index) => {

            if (element.product == element2.prod_name || (!element2.prod_code && index == 0)) {
              awcCasinoList_9ex.push(element2);
            }
          });
        });
        this.awcCasinoList_skyfair.forEach(element2 => {
          resp.result.forEach((element, index) => {
            if (element.product == element2.prod_name || (!element2.prod_code && index == 0)) {
              awcCasinoList_skyfair.push(element2);
            }
          });
        });

        this.awcCasinoList_9ex = awcCasinoList_9ex;
        this.awcCasinoList_skyfair = awcCasinoList_skyfair;
      }
    })
  }
